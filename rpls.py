...
# Welcome page
def welcome():
    # destroy function to destroy the welcome page widgets and  rewrite on the frame or screen
    def destroy(event):
        global UserName
        UserName = username_entry.get()
        if UserName == "":
            messagebox.showinfo(title="Oops", message="Please do not leave the Username empty")
            return

        for widget in window.winfo_children():
            widget.destroy()
        window.config(padx=0, pady=0, bg="#F0F0F0")
        main_game()

    window.config(padx=50, pady=10, bg="#F6EABE")
    welcome_label = Label(text="WELCOME", bg="#F6EABE", font=("Comic Sans MS", 40, "normal"))
    welcome_label.pack()

    to_label = Label(text="TO", bg="#F6EABE", font=("Comic Sans MS", 40, "normal"))
    to_label.pack()

    rpcls_label = Label(text="🥊Rock ✋Paper ✂Scissor 🦎Lizard 🖖Spock", bg="#F6EABE", font=("Comic Sans MS", 17, "normal"))
    rpcls_label.pack()

    username_label = Label(text="Enter Username :", bg="#F6EABE", font=("Comic Sans MS", 15, "normal"))
    username_label.pack(pady=5, padx=20, side="left")

    username_entry = Entry(font=("Comic Sans MS", 12, "bold"))
...
def main_game():
    ...
    # This function will destroy the last page so that welcome page can be displayed again
    def play_again():
        global win
        global loss
        global tie
        ...
        global winPercent
        global gamesPlayed
        win = loss = tie = winPercent = gamesPlayed = 0

        # asking user if he want to continue with the same username or not
        if not messagebox.askyesno(title="?", message="Do you want to continue with the same username"):
            UserName = ""

        # destroying result page widgets
        for widget in window.winfo_children():
            widget.destroy()

        # calling welcome page or main page depending on user earlier choice
        if UserName == "":
            welcome()
        else:
            window.config(padx=0, pady=0, bg="#F0F0F0")
    ...
    # this function will execute when the 5 rounds will be completed
    def game_completed():
        for widget in window.winfo_children():
            widget.destroy()
        bg_color = ""
        ...
        congrats_lable = Label(fg="white", bg=bg_color, text=(message + " " + UserName + " " + logo) , font=('Comic Sans MS', 25, 'bold'))
        congrats_lable.grid(row=0, column=0, columnspan=2, padx=(150, 150))
        user_result = Label(fg="white", bg=bg_color, text=f"You {message2}", font=('Comic Sans MS', 18, 'bold'))
        user_result.grid(row=1, column=0, columnspan=2)
        wins = Label(fg="white", bg=bg_color, text=f"Your wins = {win}/{gamesPlayed}", font=('Comic Sans MS', 14, 'bold'))
        wins.grid(row=2, column=0)
        cWins = Label(fg="white", bg=bg_color, text=f"Computer wins = {loss}/{gamesPlayed}", font=('Comic Sans MS', 14, 'bold'))
        cWins.grid(row=2, column=1)

        play_againBtn = Button(bg="green", text="Play Again 🔄", command=play_again, font=('Courier', 15, 'bold'))
        play_againBtn.grid(row=3, column=0, pady=70)

        Exit = Button(bg="red", width=10, text="Exit 👋", command=window.destroy, font=('Courier', 17, 'bold'))
        Exit.grid(row=3, column=1, pady=70)
    ...
    def game(player):
        ...
        #asking user to confirm its choice
        is_ok = messagebox.askyesno(title="Confirm Choice", message=f"You chose {player} \n Press yes to confirm your choice:")
        if not is_ok:
            return
    ...
    username_label = Label(topframe, text=f"Welcome {UserName}", fg="#FF0000", font=('Comic Sans MS', 15, 'bold'))
    username_label.pack()
